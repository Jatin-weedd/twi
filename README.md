# 🏔️ India TWI Data Gateway (Render)

This is **not a rendering API**. It's a thin, authenticated, metered
gatekeeper in front of the raw private TWI COG tiles. Clients get the raw
...
**Data stays on Hugging Face.** The 9 TWI tiles remain in the private
dataset repo `J2003S/india-twi`. This gateway never downloads a whole tile
...
This Gateway (Render)
        ...
        │  Range: bytes=1048576-1056767
        ▼
Private HF dataset: J2003S/india-twi/TWI_Tile3_COG.tif
```
...
**Also known:** the `/api/v1/resolve/tile/{filename}` and
`/api/v1/resolve/watershed` endpoints hand back a resolved Hugging Face URL
instead of proxying bytes. Once a client has that URL, the range cap and
byte quota above no longer apply to what they do with it — treat those two
routes as a separate, less-metered access path, not a version of the same
guarantee.
...
|---|---|---|
| `HF_TOKEN` | Yes | Read access to the private dataset repo `J2003S/india-twi`. |
...
with rasterio.Env(**gdal_opts):
    with rasterio.open("/vsicurl/https://<your-service>.onrender.com/tiles/TWI_Tile1_COG.tif") as src:
...
const tiff = await GeoTIFF.fromUrl(
  "https://<your-service>.onrender.com/tiles/TWI_Tile1_COG.tif",
...
Same as before: 9 tiles (`TWI_Tile1_COG.tif` … `TWI_Tile9_COG.tif`) in
`J2003S/india-twi`, pre-buffered 300 m on every side so adjacent tiles
...
- The `/api/v1/resolve/...` endpoints bypass the range cap and byte quota
  entirely once a URL is handed out (see above) — don't treat them as
  covered by the same guarantees as `/tiles/{filename}`.
